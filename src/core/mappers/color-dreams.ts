 ...
 *   - bit 0-1: PRG バンク番号
 *   - bit 4-7: CHR バンク番号
 * - バスコンフリクトあり (実機ではAND演算されるが多くのゲームで問題にならない)
 * 仕様参照: https://www.nesdev.org/wiki/Color_Dreams
 */
...
export class MapperColorDreams implements Mapper {
  ...
  writePrg(_addr: number, value: number): void {
    this.prgBankOffset = (value & this.prgBankMask) * PRG_BANK_SIZE;
    this.chrBankOffset = ((value >> 4) & this.chrBankMask) * CHR_BANK_SIZE;
  }
