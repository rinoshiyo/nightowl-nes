...
const en: Translations = {
  ...
  emulator: {
    ...
    save: `Save (Slot 1~${stats.saveSlots})`,
    load: `Load (Slot 1~${stats.saveSlots})`,
  ...
  quality: {
    ...
    subtitle: `${fmt(stats.tsLines)} lines of TypeScript. ${fmt(stats.testPass)} tests. Zero runtime dependencies. Verified against the NES hardware specification.`,
    cpuLabel: "CPU",
    cpuDesc: `${fmt(stats.opcodesImplemented)} of ${fmt(stats.opcodesTotal)} opcodes — ${fmt(stats.opcodesOfficial)} official + ${fmt(stats.opcodesUnofficial)} unofficial — implemented from the 6502 specification alone.`,
    ppuLabel: "PPU",
    ...
    mapperLabel: "Mappers",
    mapperDesc: `${fmt(stats.mapperCount)} mappers covering ~85–90% of the commercial NES library.`,
    testLabel: "Tests",
    testDesc: `${fmt(stats.testPass)} pass / ${fmt(stats.testSkip)} skip / ${fmt(stats.testFail)} fail. CI on every commit.`,
    assertLabel: "Assertions",
    assertDesc: `${fmt(stats.expects)} expect() calls verifying correctness at opcode, register, and pixel level.`,
    nestestLabel: "nestest",
    nestestDesc: `The gold-standard CPU verification ROM. ${fmt(stats.traceLines)} trace lines matched. Passed.`,
    romLabel: "Test ROMs",
    romDesc: `${fmt(stats.romPassTotal)} of ${fmt(stats.romTotal)} ROMs from nes-test-roms. CPU ${fmt(stats.romPassCpu)} / PPU ${fmt(stats.romPassPpu)} / APU ${fmt(stats.romPassApu)} / Mapper ${fmt(stats.romPassMapper)} / others ${fmt(stats.romPassOthers)}. ${fmt(stats.romRemaining)} still to go.`,
    sourceLabel: "Source",
    sourceDesc: `${fmt(stats.tsLines)} lines of TypeScript. Not a line more than needed.`,
  ...
  harness: {
    ...
    loopDesc: `Shell scripts and event-driven hooks orchestrate the cycle: worker completes Issue → writes flag → Stop hook fires → loop-helper runs /clear → fresh session picks up next open Issue. Code review runs 7-angle finders under a bot identity. Stuck protocol drafts PRs back after ${stats.stuckTimeoutMin} min. Auto-recover restarts from crashes.`,
    prsLabel: "merged PRs",
    ...
    denyDesc: `All existing NES emulator source code — ${fmt(stats.denyListRepos)} known repositories, any language, any license.`,
    ...
    wipNote: `${fmt(stats.romPassTotal)} of ${fmt(stats.romTotal)} test ROMs passing. ${fmt(stats.romRemaining)} still to go. This project is under active development.`,
  },
  footer: {
    license: "MIT License",
...
const ja: Translations = {
  ...
  emulator: {
    ...
    direction: "方向",
    fullscreen: "フルスクリーン",
    save: `セーブ (Slot 1~${stats.saveSlots})`,
    load: `ロード (Slot 1~${stats.saveSlots})`,
  ...
  quality: {
    ...
    subtitle: `TypeScript ${fmt(stats.tsLines)}行。テスト ${fmt(stats.testPass)}本。ランタイム依存ゼロ。NES ハードウェア仕様に照らして検証済み。`,
    cpuLabel: "CPU",
    cpuDesc: `${fmt(stats.opcodesTotal)}オプコード中${fmt(stats.opcodesImplemented)}実装 — 公式${fmt(stats.opcodesOfficial)} + 非公式${fmt(stats.opcodesUnofficial)} — 6502仕様書のみから実装。`,
    ppuLabel: "PPU",
    ppuDesc: "背景 + スプライト描画、loopy スクロールレジスタ、Sprite 0 Hit 検出、スキャンライン精度タイミング。",
    apuLabel: "APU",
    apuDesc: "全5チャンネル — Pulse×2、Triangle、Noise、DMC — 非線形ミキシング。",
    mapperLabel: "Mappers",
    mapperDesc: `${fmt(stats.mapperCount)}種のマッパーで商用 NES ライブラリの約85〜90%をカバー。`,
    testLabel: "テスト",
    testDesc: `${fmt(stats.testPass)} pass / ${fmt(stats.testSkip)} skip / ${fmt(stats.testFail)} fail。毎コミット CI で検証。`,
    assertLabel: "アサーション",
    assertDesc: `${fmt(stats.expects)}回の expect() でオプコード・レジスタ・ピクセルレベルの正確性を検証。`,
    nestestLabel: "nestest",
    nestestDesc: `CPU 検証の最高基準 ROM。${fmt(stats.traceLines)}行のトレースを照合。合格。`,
    romLabel: "テスト ROM",
    romDesc: `nes-test-roms から${fmt(stats.romTotal)}個中${fmt(stats.romPassTotal)}個を使用。CPU ${fmt(stats.romPassCpu)} / PPU ${fmt(stats.romPassPpu)} / APU ${fmt(stats.romPassApu)} / Mapper ${fmt(stats.romPassMapper)} / その他 ${fmt(stats.romPassOthers)}。残り${fmt(stats.romRemaining)}。`,
    sourceLabel: "ソース",
    sourceDesc: `TypeScript ${fmt(stats.tsLines)}行。必要以上に1行も多くない。`,
    depsLabel: "依存",
  ...
  harness: {
    ...
    loopTitle: "自走ループ",
    loopDesc: `シェルスクリプトとイベント駆動 hook がサイクルを回す——ワーカーが Issue を完了 → フラグ書込 → Stop hook 検出 → loop-helper が /clear → 新セッションが次の open Issue を取得。コードレビューは bot 名義で7角度の finder を実行。stuck プロトコルで${stats.stuckTimeoutMin}分停滞を検出。auto-recover でクラッシュから自動復旧。`,
    prsLabel: "merged PRs",
    issuesLabel: "Issues closed",
    daysLabel: "日間",
    nesPrsLabel: "NES",
    ...
    denyTitle: "参照 NG",
    denyDesc: `既存 NES エミュレータのソースコード全て — ${fmt(stats.denyListRepos)}リポジトリ、言語・ライセンス不問。`,
    enforceDesc: "pre-tool-use hooks が deny list の WebFetch URL をブロック。構造的な制約であり、完全な遮断ではない——WebSearch 結果やローカルファイルはブロックされない。",
    wipNote: `${fmt(stats.romTotal)}個中${fmt(stats.romPassTotal)}個のテスト ROM が合格。残り${fmt(stats.romRemaining)}個。このプロジェクトは開発中。`,
  },
  footer: {
    license: "MIT License",
...
const zh: Translations = {
  ...
  emulator: {
    ...
    direction: "方向",
    fullscreen: "全屏",
    save: `保存 (Slot 1~${stats.saveSlots})`,
    load: `读取 (Slot 1~${stats.saveSlots})`,
  ...
  quality: {
    ...
    subtitle: `${fmt(stats.tsLines)}行 TypeScript。${fmt(stats.testPass)}项测试。零运行时依赖。经 NES 硬件规范验证。`,
    cpuLabel: "CPU",
    cpuDesc: `${fmt(stats.opcodesTotal)}个操作码中实现${fmt(stats.opcodesImplemented)}个 — 官方${fmt(stats.opcodesOfficial)} + 非官方${fmt(stats.opcodesUnofficial)} — 仅依据6502规范实现。`,
    ppuLabel: "PPU",
    ppuDesc: "背景 + 精灵渲染、loopy滚动寄存器、Sprite 0 Hit检测、扫描线精确时序。",
    apuLabel: "APU",
    apuDesc: "全部5个通道 — Pulse×2、Triangle、Noise、DMC — 非线性混音。",
    mapperLabel: "Mappers",
    mapperDesc: `${fmt(stats.mapperCount)}种映射器，覆盖约85–90%的商用NES游戏库。`,
    testLabel: "测试",
    testDesc: `${fmt(stats.testPass)}通过 / ${fmt(stats.testSkip)}跳过 / ${fmt(stats.testFail)}失败。每次提交均通过 CI 验证。`,
    assertLabel: "断言",
    assertDesc: `${fmt(stats.expects)}次expect()调用，在操作码、寄存器和像素级别验证正确性。`,
    nestestLabel: "nestest",
    nestestDesc: `CPU验证黄金标准ROM。${fmt(stats.traceLines)}行追踪比对。通过。`,
    romLabel: "测试 ROM",
    romDesc: `使用 nes-test-roms 中${fmt(stats.romTotal)}个ROM中的${fmt(stats.romPassTotal)}个。CPU ${fmt(stats.romPassCpu)} / PPU ${fmt(stats.romPassPpu)} / APU ${fmt(stats.romPassApu)} / Mapper ${fmt(stats.romPassMapper)} / 其他 ${fmt(stats.romPassOthers)}。剩余${fmt(stats.romRemaining)}个。`,
    sourceLabel: "源码",
    sourceDesc: `${fmt(stats.tsLines)}行TypeScript。不多一行。`,
    depsLabel: "依赖",
  ...
  harness: {
    ...
    loopTitle: "自驱动循环",
    loopDesc: `Shell脚本和事件驱动hook编排周期——工作者完成Issue → 写入标志 → Stop hook触发 → loop-helper执行/clear → 新会话获取下一个open Issue。代码审查以bot身份运行7角度finder。stuck协议在${stats.stuckTimeoutMin}分钟无进展后退回draft。auto-recover从崩溃中自动恢复。`,
    prsLabel: "merged PRs",
    issuesLabel: "Issues closed",
    daysLabel: "天",
    nesPrsLabel: "NES",
    ...
    denyTitle: "禁止参考",
    denyDesc: `所有现有NES模拟器源代码 — ${fmt(stats.denyListRepos)}个已知仓库，不限语言、不限许可证。`,
    enforceDesc: "pre-tool-use hooks阻止deny list中的WebFetch URL。结构性约束，非完全封锁——WebSearch结果和本地文件不会被阻止。",
    wipNote: `${fmt(stats.romTotal)}个测试ROM中${fmt(stats.romPassTotal)}个通过。剩余${fmt(stats.romRemaining)}个。本项目正在积极开发中。`,
  },
  footer: {
    license: "MIT License",
