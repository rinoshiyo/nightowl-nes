...
describe("NesBus RAM", () => {
  it("$0000-$07FF に read/write できる", () => {
    const bus = new NesBus(new Ppu(), createMapper(makeDummyCart()), new Controller(), new Apu());
...
describe("NesBus Cart", () => {
  ...
  it("16KB PRG が $C000 にミラーされる", () => {
    const bus = new NesBus(new Ppu(), createMapper(makeDummyCart(0x4000)), new Controller(), new Apu());
