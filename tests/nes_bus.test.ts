...
describe("NesBus RAM", () => {
  it("$0000-$07FF に read/write できる", () => {
    const bus = new NesBus(new Ppu(), createMapper(makeDummyCart()), new Controller());
...
describe("NesBus PPU dispatch", () => {
  it("$2000 write が PPU CTRL に反映される", () => {
    const ppu = new Ppu();
    const bus = new NesBus(ppu, createMapper(makeDummyCart()), new Controller());
...
describe("NesBus Cart", () => {
  ...
  it("16KB PRG が $C000 にミラーされる", () => {
    const bus = new NesBus(new Ppu(), createMapper(makeDummyCart(0x4000)), new Controller());
