...
describe("illegal SLO (*SLO)", () => {
  describe("$07 zeroPage (2 byte, 5 cycle)", () => {
    it("shifts memory left and ORs with A, sets C from bit7", () => {
    ...
    it("clears C when bit7 of original value is 0", () => {
      const { bus, ram } = makeRamBus();
      ram[0x0200] = 0x07;
      ram[0x0201] = 0x47;
      ram[0x0047] = 0x37;
      ...
      cpuStep(cpu, bus);
      expect(ram[0x0047]).toBe(0x6e);
      expect(cpu.a).toBe(0x7e);
      expect(cpu.p & CpuFlags.C).toBe(0);
    });

    it("sets Z flag when ORA result is zero", () => {
      const { bus, ram } = makeRamBus();
      ram[0x0200] = 0x07;
      ram[0x0201] = 0x47;
      ram[0x0047] = 0x00;
    ...
    it("wraps 0x80 shift to 0x00 with C set", () => {
      const { bus, ram } = makeRamBus();
      ram[0x0200] = 0x07;
      ram[0x0201] = 0x47;
      ram[0x0047] = 0x80;
  ...
  describe("$03 (indirect,X) (2 byte, 8 cycle)", () => {
    it("ASL+ORA via indexed indirect", () => {
      const { bus, ram } = makeRamBus();
      ram[0x0200] = 0x03;
      ram[0x0201] = 0x45;
      ram[0x0047] = 0x47;
  ...
  describe("$0F absolute (3 byte, 6 cycle)", () => {
    it("ASL+ORA at absolute address", () => {
      const { bus, ram } = makeRamBus();
      ram[0x0200] = 0x0f;
      ram[0x0201] = 0x47;
      ram[0x0202] = 0x06;
  ...
  describe("$13 (indirect),Y (2 byte, 8 cycle)", () => {
    it("ASL+ORA via indirect indexed Y", () => {
      const { bus, ram } = makeRamBus();
      ram[0x0200] = 0x13;
      ram[0x0201] = 0x45;
      ram[0x0045] = 0x48;
  ...
  describe("$17 zeroPage,X (2 byte, 6 cycle)", () => {
    it("ASL+ORA at zeroPage+X", () => {
      const { bus, ram } = makeRamBus();
      ram[0x0200] = 0x17;
      ram[0x0201] = 0x48;
      ram[0x0047] = 0x29;
      const cpu = createCpu({ pc: 0x0200, a: 0xc3, x: 0xff, p: CpuFlags.U | CpuFlags.I });

      const used = cpuStep(cpu, bus);
      expect(used).toBe(6);
      expect(ram[0x0047]).toBe(0x52);
      expect(cpu.a).toBe(0xd3);
    });
  });

  describe("$1B absolute,Y (3 byte, 7 cycle)", () => {
    it("ASL+ORA at absolute+Y", () => {
      const { bus, ram } = makeRamBus();
      ram[0x0200] = 0x1b;
      ram[0x0201] = 0x48;
      ram[0x0202] = 0x05;
  ...
  describe("$1F absolute,X (3 byte, 7 cycle)", () => {
    it("ASL+ORA at absolute+X", () => {
      const { bus, ram } = makeRamBus();
      ram[0x0200] = 0x1f;
      ram[0x0201] = 0x48;
      ram[0x0202] = 0x05;
