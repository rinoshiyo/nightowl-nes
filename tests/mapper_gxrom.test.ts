...
function makeGxromCart(prgBankCount: number, chrBankCount: number): Cart {
...
describe("MapperGxrom PRG bank switching", () => {
  it("初期状態でバンク 0 が選択されている", () => {
    const mapper = new MapperGxrom(makeGxromCart(4, 4));
    expect(mapper.readPrg(0x8000)).toBe(0);
    expect(mapper.readPrg(0x8001)).toBe(0xaa);
  });

  it("bit 4-5 で PRG バンクが切り替わる", () => {
    const mapper = new MapperGxrom(makeGxromCart(4, 4));
  ...
  it("PRG バンク番号はバンク数でマスクされる", () => {
    const mapper = new MapperGxrom(makeGxromCart(2, 4));
    mapper.writePrg(0x8000, 0x30);
    expect(mapper.readPrg(0x8000)).toBe(1);
  });
});

describe("MapperGxrom CHR bank switching", () => {
  it("初期状態でバンク 0 が選択されている", () => {
    const mapper = new MapperGxrom(makeGxromCart(4, 4));
  ...
  it("CHR バンク番号はバンク数でマスクされる", () => {
    const mapper = new MapperGxrom(makeGxromCart(4, 2));
    mapper.writePrg(0x8000, 0x03);
    expect(mapper.readChr(0x0000)).toBe(1);
  });

  it("PRG と CHR を同時に切替 (bit レイアウトが Color Dreams と逆)", () => {
    const mapper = new MapperGxrom(makeGxromCart(4, 4));
...
describe("MapperGxrom serialize/deserialize", () => {
  it("状態の保存・復元が正しく動作", () => {
    const mapper = new MapperGxrom(makeGxromCart(4, 4));
    mapper.writePrg(0x8000, 0x12);
    const state = mapper.serializeMapper();

    const mapper2 = new MapperGxrom(makeGxromCart(4, 4));
...
describe("MapperGxrom createMapper 統合", () => {
  it("createMapper で mapper 66 が MapperGxrom にマッピングされる", () => {
    const cart = makeGxromCart(4, 4);
    const mapper = createMapper(cart);
    expect(mapper).toBeInstanceOf(MapperGxrom);
  });

  it("mapperId() が 66 を返す", () => {
    const mapper = new MapperGxrom(makeGxromCart(4, 4));
    expect(mapper.mapperId()).toBe(66);
  });
});
