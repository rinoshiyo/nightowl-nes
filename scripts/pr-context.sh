...
PR_JSON=$(gh pr list -s open --json number,title,isDraft,mergeStateStatus,body,headRefName -L 1 2>/dev/null || echo '[]')
...
BODY=$(printf '%s' "$PR_JSON" | jq -r '.[0].body // ""' | head -50)

# 直近のレビューコメント (bot 名義) を取得
LAST_REVIEW=$(gh pr view "$NUMBER" --comments --json comments \
  -q '[.comments[] | select(.author.login == "rinoshiyo-bot-reviewer[bot]")] | last // empty | .body // ""' \
